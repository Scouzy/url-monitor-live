...
const MEP_CRITICITE = {
  standard:        { label: "Standard",       color: "#34D399", bg: "rgba(52,211,153,0.08)",   border: "rgba(52,211,153,0.3)",   desc: "Processus complet · faible risque · planifié" },
  normal:          { label: "Normal",          color: "#818CF8", bg: "rgba(129,140,248,0.08)",  border: "rgba(129,140,248,0.3)",  desc: "Processus standard · risque maîtrisé" },
  normal_urgent:   { label: "Normal-Urgent",   color: "#FB923C", bg: "rgba(251,146,60,0.08)",   border: "rgba(251,146,60,0.3)",   desc: "Urgence métier · processus allégé" },
  standard_urgent: { label: "Standard-Urgent", color: "#F87171", bg: "rgba(248,113,113,0.08)",  border: "rgba(248,113,113,0.3)",  desc: "Urgence critique · rigueur maximale" },
};
...
const MEP_TEMPLATES = {
  standard: [
    { title: "Vérification du bilan de qualification",    type: "check",    phase: "pre",  responsible: "Dev / IT",        duration: "15min" },
    { title: "Communication de la fenêtre de MeP",       type: "notify",   phase: "pre",  responsible: "Chef de projet",   duration: "10min" },
    { title: "Validation Go/No-Go",                      type: "approval", phase: "pre",  responsible: "Responsable MeP",  duration: "15min" },
    { title: "Sauvegarde complète / plan de rollback",   type: "action",   phase: "pre",  responsible: "IT",               duration: "30min" },
    ...
    { title: "Application des scripts SQL / migrations", type: "script",   phase: "mep",  responsible: "DBA",              duration: "15min" },
    { title: "Mise à jour des configurations",           type: "action",   phase: "mep",  responsible: "IT",               duration: "10min" },
    { title: "Redémarrage des services",                 type: "action",   phase: "mep",  responsible: "IT",               duration: "5min"  },
    { title: "Smoke tests / validation fonctionnelle",   type: "check",    phase: "post", responsible: "Dev / MOA",        duration: "30min" },
    { title: "Vérification des logs applicatifs",        type: "check",    phase: "post", responsible: "IT",               duration: "15min" },
    { title: "Validation des performances",              type: "check",    phase: "post", responsible: "Dev / IT",         duration: "20min" },
    { title: "Communication de clôture et PV de MeP",   type: "notify",   phase: "post", responsible: "Chef de projet",   duration: "10min" },
  ],
  normal: [
  ...
  normal_urgent: [
    { title: "Notification d'urgence aux équipes",       type: "notify",   phase: "pre",  responsible: "Chef de projet",   duration: "5min"  },
    { title: "Sauvegarde rapide",                        type: "action",   phase: "pre",  responsible: "IT",               duration: "10min" },
    { title: "Déploiement du correctif",                 type: "script",   phase: "mep",  responsible: "Dev / IT",         duration: "15min" },
    { title: "Redémarrage des services",                 type: "action",   phase: "mep",  responsible: "IT",               duration: "5min"  },
    { title: "Validation rapide",                        type: "check",    phase: "post", responsible: "Dev / MOA",        duration: "15min" },
    { title: "Communication de résolution",              type: "notify",   phase: "post", responsible: "Chef de projet",   duration: "5min"  },
  ],
  standard_urgent: [
    { title: "Approbation urgente Go/No-Go",             type: "approval", phase: "pre",  responsible: "Responsable MeP",  duration: "10min" },
    { title: "Notification d'urgence",                   type: "notify",   phase: "pre",  responsible: "Chef de projet",   duration: "5min"  },
    { title: "Sauvegarde + plan de rollback immédiat",   type: "action",   phase: "pre",  responsible: "IT",               duration: "20min" },
    ...
    { title: "Tests de non-régression prioritaires",     type: "check",    phase: "post", responsible: "Dev",              duration: "20min" },
...
function MepEditor({ sel, upd, addStep, delStep, moveStep, updStep, toggleExp, expSteps }) {
  ...
  return (
    <div style={{ flex: 1, overflowY: "auto", padding: "0 0 48px" }}>
      ...
      {/* Bandeau criticité */}
      <div style={{ background: crit.bg, borderBottom: `2px solid ${crit.border}`, padding: "14px 32px", display: "flex", alignItems: "center", gap: 14, flexWrap: "wrap" }}>
        <Rocket size={18} color={crit.color} />
        <div style={{ flex: 1 }}>
        ...
        <select value={sel.criticite} onChange={e => upd({ criticite: e.target.value })}
          ...
          {Object.entries(MEP_CRITICITE).map(([k, v]) => <option key={k} value={k} style={{ color: "#E5E7EB" }}>{v.label}</option>)}
        </select>
        <select value={sel.status} onChange={e => upd({ status: e.target.value })}
